...
class ForumUsersSeeder extends Seeder
{
    ...
    public function run(): void
{
    $styles = ['adventurer', 'avataaars', 'bottts', 'lorelei'];
    $users = [];

    $usernames = ['Juan', 'Ana', 'Carlos', 'Luis', 'María', 'Pedro', 'Laura', 
                 'Diego', 'Sofía', 'Javier', 'Elena', 'Miguel', 'Lucía', 
                 'Daniel', 'Paula', 'Andrés', 'Isabel', 'Fernando'];

    foreach ($usernames as $username) {
        $style = $styles[array_rand($styles)];
        $users[] = [
            'username' => $username,
            'profile_pic' => "https://api.dicebear.com/7.x/$style/svg?seed=$username",
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }

    DB::table('forum_users')->insert($users);
}
}
